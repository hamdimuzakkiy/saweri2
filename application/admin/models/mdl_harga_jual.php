...
class mdl_harga_jual extends CI_Model{
	...
	{
		$this->db->flush_cache();
		$this->db->select('harga.id_harga, barang.nama_barang AS nama_barang, harga.periode_start, harga.periode_end, harga.harga_jual');
		$this->db->from('harga');
		$this->db->join('barang', 'barang.id_barang = harga.id_barang');
		$this->db->where('harga.harga_jual >', '0');
