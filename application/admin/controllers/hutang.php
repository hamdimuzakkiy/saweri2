...
class hutang extends My_Controller
{
	...
	function pembayaran(){		
		...
		$data['can_view'] 	= $this->can_view();		
		$data['can_insert'] = $this->can_insert();		
		$data['can_update'] = $this->can_update();		$data['can_delete'] = $this->can_delete();				
		$this->open();				
		...
		$config['next_tag_close'] = '</li>';				$config['last_link'] = 'Last';		
		...
		$data['hpp'] = $this->input->post('hpp');		$data['harga_toko'] = $this->input->post('harga_toko');		
		$data['harga_partai'] = $this->input->post('harga_partai');		$data['harga_cabang'] = $this->input->post('harga_cabang');				
		$data['is_hargatoko'] = $this->input->post('is_hargatoko');		
		$data['is_hargapartai'] = $this->input->post('is_hargapartai');		
		$data['is_hargajual'] = $this->input->post('is_hargajual');		$data['point_karyawan'] = $this->input->post('point_karyawan');		
		$data['point_member'] = $this->input->post('point_member');		$data['userid'] = get_userid();						
		...
		}else{				$this->hutang->insert($data);						
		$this->session->set_flashdata('message', 'Data Penerimaan Kas Berhasil disimpan.');			
		redirect('hutang');		}				$this->close();	}		function update($id)	{		
		$glid1=$this->uri->segment(3);		$glid2=$this->uri->segment(4);		
		$glid3=$this->uri->segment(5);		$id = $glid1 . '/' . $glid2 . '/' . $glid3;		
		$config['base_url'] = base_url().'index.php/hutang/index/';		
		$config['total_rows'] = $this->db->count_all('daftar_hutang');		
		$config['per_page'] = '20';		$config['num_links'] = '5';		$config['uri_segment'] = '3';				
		$config['full_tag_open'] = '';		$config['full_tag_close'] = '';				
		$config['num_tag_open'] = '<li>';		$config['num_tag_close'] = '</li>';				
		$config['cur_tag_open'] = '<li><a href="javascript:void(0)" class="current">';		
		$config['cur_tag_close'] = '</a></li>';				$config['prev_link'] = 'Prev';		
		...
		/*end list angsuran*/				if ($this->can_update() == FALSE){			
	...
	function process_update()	{		if ($this->can_update() == FALSE){			
		redirect('auth/failed');		}		$this->open();		
		...
		$data['tanggal'] = $this->input->post('tanggal');		$data['glid'] = $this->input->post('glid');				
		$data['id_coa'] = '5';		
		...
		$angsuran_hutang['TANGGAL'] 	= date('Y-m-d'); 			
		...
		$this->db->flush_cache();						if ($sisa_hutang=='0'){				
		$update_pembelian = array(								'status_hutang' => '2'				);								
		}else{				
		$update_pembelian = array(								'status_hutang' => '1'				);					
