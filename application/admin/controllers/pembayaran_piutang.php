...
class pembayaran_piutang extends My_Controller
{
	...
	function index($sebagai='')
	{
		...
		$config['base_url'] = base_url().'index.php/pembayaran_piutang/index/';
		...
		$this->load->view('pembayaran_piutang/pembayaran_piutang_list', $data);
	...
	$this->load->view('pembayaran_piutang/pembayaran_piutang_add',$data);					
	}else{				
	$this->pembayaran_piutang->insert($data);						
	$this->session->set_flashdata('message', 'Data Penerimaan Kas Berhasil disimpan.');			
	redirect('pembayaran_piutang');		}				
	$this->close();	
	}
