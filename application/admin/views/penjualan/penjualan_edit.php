	...
	<div class="block-border">
		...
		?>
			...
			<fieldset >
				...
				<div class="columns">
					<input type="hidden" name="id_penjualan" value="<?=$result->row()->id_penjualan?>" >
					<p class="colx2-left">
						<label for="complex-en-url">SO No :</label>
						<span class="relative">
							<input type="text" name="so_no" id="so_no" value="<?=$result->row()->so_no?>" class="satu-width">
						</span>
					</p>
					<p class="colx2-right">
				...
				<div class="columns">
							...
							<select name="id_pelanggan" id="id_pelanggan" >
								<?php
									...
									if($query->num_rows() > 0)
									{
										...
										{
											if($row->id_pelanggan == $result->row()->id_pelanggan){
												echo '<option value="'.$row->id_pelanggan.'" selected="selected">'.$row->kode_pelanggan.' - '.$row->nama.'</option>';
											}else{
					...
					<p class="colx2-right">
						...
						<span class="relative">
							<input type="text" name="jatuh_tempo" id="jatuh_tempo" value="<?=$result->row()->jatuh_tempo?>" class="duapertiga-width"> Hari
						</span>
					</p>					
				</div>
				...
				<div class="columns">
					...
					<p class="colx2-right">
						...
						<span class="relative">
							<input type="text" name="diskon" id="diskon" value="<?=$result->row()->diskon?>" class="duapertiga-width">
						</span>
					</p>
				</div>
				<div class="columns">
							...
							<select name="id_jenis_penjualan" id="id_jenis_penjualan" onChange="javascript:set_jenispenjualan(this.value)">
								<?php
									...
									if($query->num_rows() > 0)
									{
										foreach($query->result() as $row)
										{
											if($result->row()->id_jpenjualan == $row->id_jpenjualan){
						...
						<table class="table" border="1" cellspacing="0" width="100%">
							...
							<tbody id="detail">
								<?php
									...
									$query = $this->penjualan->get_detail($id_penjualan);
									$i = 0;
										...
										echo '
												<tr>
													...
													<td>
														...
														<input type="hidden" name="detail['.$i.'][id_detail_pembelian]" value="'.$row->id_detail_pembelian.'" />
													</td>
													<td>
													...
													<td>
														'.$row->sn.'
														<input type="hidden" name="detail['.$i.'][sn]" value="'.$row->sn.'" />
													</td>
													<td>
														'.convert_rupiah($row->harga).'
														<input type="hidden" name="detail['.$i.'][total]" value="'.$row->harga.'" />
													</td>
													<td align="center">
