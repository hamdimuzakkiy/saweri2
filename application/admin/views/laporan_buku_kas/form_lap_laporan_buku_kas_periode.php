<script type="text/javascript">
	...
	function laporan(pageURL, title,w,h){
		...
		if ((periode_awal=='')||(periode_akhir=='')){
			...
			var targetWin = window.open ('<?=site_url();?>/laporan_buku_kas/view_lap_buku_kas_periode/'+periode_awal+'/'+periode_akhir, title, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
...
<section class="grid_12">
	<div class="block-border">
		<?php
			...
			echo form_open('laporan_buku_kas/view_lap_buku_kas_periode', $attributes);
		?>
			<h1>Form Laporan Buku Kas [Periode]</h1>
			
			<fieldset>
				<div class="columns">
					<input type="hidden" name="id_buku_kas" id="id_buku_kas" value="<?=date('YmdHis')?>">
					<p class="colx2-left">
			...
			<div id="tab-settings" class="tabs-content">
					<button type="button" onclick="laporan('<?=site_url();?>/laporan_buku_kas/view_lap_buku_kas_periode/','name',1000,800)"><img src="<?=base_url()?>asset/admin/images/icons/fugue/tick-circle.png" width="16" height="16">Print Preview</button> 
