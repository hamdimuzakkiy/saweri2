...
<section class="grid_12">
	<div class="block-border">
		<?php
			...
			echo form_open('service/process_bayar', $attributes);
		?>
			...
			<fieldset>
				...
				<div class="columns">
					<p class="colx2-left">
						...
						<span class="relative">							<input type="text" name="nama_pelanggan" value="<?=$nama_pelanggan?>">							
							<!--<select name="nama_pelanggan" id="nama_pelanggan" readonly="true" class="setengah-width ">-->
								<?php /*
				...
				<div class="columns">
					...
					<p class="colx2-right">
						...
						<span class="relative">
							<select name="status"  id="status"  class="duapertiga-width">										<?php																		$query = $this->db->get('service_status');									if($query->num_rows() > 0)									{										foreach($query->result() as $row)										{											if($status == $row->id){												echo '<option value="'.$row->id.'" selected="selected">'.$row->deskripsi.'</option>';											}else{												echo '<option value="'.$row->id.'" >'.$row->deskripsi.'</option>';											}										}									}								?>									
							</select>
						</span>
					</p>
				</div>
