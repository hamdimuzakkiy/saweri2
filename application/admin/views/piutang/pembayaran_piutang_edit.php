<script type="text/javascript">
	...
	function batal(){
		document.location.href = '<?=base_url().'index.php/piutang/pembayaran'?>';
	}
...
<script type="text/javascript">
	...
	function remove_detail(id){
		...
		tanggal.focus();
	}			function bayar_piutang(){		var var_pembayaran;		var var_total_tagiahan;		var sisa;				var_pembayaran=document.getElementById("pembayaran").value;		var_total_tagiahan=document.getElementById("total_tagihan").value;		sisa=var_total_tagiahan - var_pembayaran;				document.getElementById('sisa').value = sisa;					}
...
<section class="grid_12">	<div class="block-border">		<form class="block-content form"  name="table_form" id="table_form" method="post" action="">			<h1>Piutang > Historical Pembayaran Piutang</h1>						<div class="block-controls">								<ul class="controls-buttons">					<?php echo $this->pagination->create_links(); ?>					<li class="sep"></li>					<li></li>				</ul>							</div>					<div class="no-margin"><table class="table" cellspacing="0" width="100%">							<thead>					<tr>						<th align="left" valign="top" scope="col">GLID</th>						<th align="left" valign="top" scope="col">Tanggal</th>												<th align="left" valign="top" scope="col">Penjual</th>						<th align="left" valign="top" scope="col">Pembeli</th>						<th align="left" valign="top" scope="col">Total (Rp)</th>						<th align="left" valign="top" scope="col">Angsuran (Rp)</th>						<th align="left" valign="top" scope="col">Sisa (Rp)</th>						<!--<th align="left" valign="top" scope="col">Saldo</th>-->					</tr>				</thead>								<tbody>										<?php foreach($results->result() as $row) {?>					<tr>						<td align="left" valign="top"><?=$row->GLID?> </td>						<td align="left" valign="top"><?=convert_date_mysql_indo($row->TANGGAL);?> </td>												<td align="left" valign="top"><?=$row->nama_cabang?> </td>						<td align="left" valign="top"><?=$row->nama_pelanggan?> </td>						<td align="left" valign="top"><?=convert_rupiah_non_rp($row->TOTAL);?> </td>						<td align="left" valign="top"><?=convert_rupiah_non_rp($row->ANGSURAN);?> </td>						<td align="left" valign="top"><?=convert_rupiah_non_rp($row->SISA);?> </td>						<!--<td align="left" valign="top"> </td>-->					</tr>					<?php } ?>									</tbody>						</table></div>						<ul class="message no-margin">				<li>&nbsp;</li>			</ul>						</form>	</div>	
	<div class="block-border">
		<?php
			$attributes = array('name' => 'form1', 'id' => 'form1', 'class'=>'block-content form');
			echo form_open('piutang/process_update', $attributes);
		?>
			<h1>Piutang > Form Data Pembayaran Piutang</h1>
			...
			<fieldset class="grey-bg margin">
				...
				<div class="columns">
					...
					<p class="colx3-left">
						...
						<span class="relative">
							<input type="text" name="so_no" id="so_no" readonly="true" value="<?=$result->row()->so_no?>">
						</span>
					</p>
				...
				<div class="columns">
						...
						<span class="relative">
								<?php
									...
									if($query->num_rows() > 0)
									{
										foreach($query->result() as $row)
										{
											if($result->row()->id_pelanggan == $row->id_pelanggan){
												//echo '<option value="'.$row->id_pelanggan.'" selected="selected">'.$row->kode_pelanggan.'-'.$row->nama.'</option>';												echo '<input type="text" name="name_pelanggan" value="' . $row->nama . '" readonly="true">';												echo '<input type="hidden" name="id_pelanggan" value="' . $row->id_pelanggan . '" readonly="true">';
											}/*else{
					...
					</p>										<p class="colx3-center">						<label for="complex-en-url">Biaya Pembayaran :</label>						<span class="relative">							<input type="text" name="pembayaran" id="pembayaran" onchange="bayar_piutang()" value="" />							<input type="hidden" name="glid" id="glid" value="<?=$result->row()->GLID?>">						</span>					</p>										
