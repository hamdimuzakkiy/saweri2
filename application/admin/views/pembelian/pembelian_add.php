...
<script type="text/javascript">
	...
	function set_detail(){		
			...
			}else if((cara_bayar=='2') && (document.getElementById('detail_jatuh_tempo').value == '')){			alert('Jatuh Tempo Tidak Boleh Kosong.');					}else if (detail_idjenis=='4'){			$.ajax({				type: 'POST',				url: '<?php echo base_url().'asset/admin/js/ajax_pembelian.php?command=add_sn'?>',				data: $('#get_serialize :input').serialize(),				success: function(data) {					$('#detail').html(data);					/*$.fancybox(data);*/				}			});					}else{
				$.ajax({
					type: 'POST',
