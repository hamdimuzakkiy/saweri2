...
<script type="text/javascript">
	...
	$(document).ready(function() {
            $.ajax({
                url: site+"/laporan_penjualan/get_member/",
            ...
            queryDelay: 200,
            innerText: "pencarian nama member",
	...
	function searchfunction(str) {
            $.ajax({
                url: site+"/laporan_penjualan/search_pelanggan/"+str,
                success: function(response){			
                $('#jero_class').html(response);
                },
				dataType:"html"  		
	...
	function laporan(pageURL, title,w,h){
		...
		var left = (screen.width/2)-(w/2);
			...
			// change form info:
			myForm.target = title;
			myForm.action = "<?=site_url();?>/laporan_penjualan/view_lap_point_member/v_html";
...
<section class="grid_12">
	<div class="block-border">
		<?php
			...
			echo form_open('laporan_penjualan/view_lap_point_member/v_html/', $attributes);
		?>
			<h1>Form Laporan Point Member</h1>
			...
			<div id="tab-settings" class="tabs-content">
					<button type="button" onclick="laporan('<?=site_url();?>/laporan_penjualan/view_lap_point_member/v_html','name',1000,800)"><img src="<?=base_url()?>asset/admin/images/icons/fugue/tick-circle.png" width="16" height="16">Print Preview</button> 
