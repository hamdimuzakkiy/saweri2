<?php ini_set("memory_limit","16M"); ?>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-15">
<style>
#div_laporan{
	margin:0 auto;
}
table.laporan {
	  border-spacing: 0;
	  margin:0 auto;
	   font-size: 0.8em;
	     font-family: sans-serif;
}

tr.border,th.th_border,td.use_border {
  border:0.5px solid #ccc;
  padding:2pt;
}


</style>
</style>

<div id="div_laporan">
	<table class="laporan" width="90%" >
		<tr><td colspan="7"><b>SAWERI GADING CELL</b></td></tr>
		<tr><td colspan="7"><b>JL. S PARMAN 18 BANYUWANGI</b></td></tr>
		<tr><td colspan="7"><b>TELP (0333)-411345</b></td></tr>
		<tr><td colspan="7"><b></b><br/></td></tr>
		<tr><td colspan="7" div align="center"><b>LAPORAN PENJUALAN [AREA]</b></td></tr>
		<tr><td colspan="7"><b>PERIODE : <?php echo $periode_awal . ' s/d ' .  $periode_akhir; ?></b></td></tr>
		<tr><td colspan="7"><b>TANGGAL CETAK : <?php echo date('d-M-Y') ?></b></td></tr>
		<tr class="border">
			<th class="th_border" scope="col">NO</th>
			<th class="th_border" scope="col">TANGGAL </th>
			<th class="th_border" scope="col">SO</th>
			<th class="th_border" scope="col">PELANGGAN</th>
			<th class="th_border" scope="col">AREA</th>
			<th class="th_border" scope="col">QTY</th>
		</tr>
		
<?php
		$j=0;
		$nama_brg = null;
		$total_brg = 0;
		$qty_perbrg = 0;
			foreach($results->result() as $row)
			{
				$j=$j+1;
				
				if ($nama_brg==null){
					$nama_brg=$row->id_area;
					
				}
			
				
				if($row->id_area != $nama_brg){		
		?>
					<tr class="border">
						<td class="use_border" colspan="5">Jumlah</td>
						<td class="use_border" align="right"><?php echo $qty_perbrg ;?></td>
					</tr>
		<?php			
						$total_brg=0;
						$qty_perbrg=0;
						$nama_brg=$row->id_area;
				}
		?>
					<tr class="border">
						<td class="use_border" align="right" valign="middle"><?=$j;?></td>
						<td class="use_border" align="center" valign="middle"><?=$row->tanggal?></td>
						<td class="use_border" align="right" valign="middle"><?php echo '\''.$row->so_no?></td>
						<td class="use_border" align="left" valign="middle"><?=$row->nama_pelanggan?></td>
						<td class="use_border" align="left" valign="middle"><?=$row->area?></td>
						<td class="use_border" align="right" valign="middle"><?=$row->qty?></td>
						<?php $qty_perbrg=$row->qty+$qty_perbrg; ?>
						<?php $sum_qty[]=$row->qty ;?>
					</tr>
		<?php
			}
		?>
				<tr class="border">
						<td class="use_border" colspan="5">Jumlah : </td>
						<td class="use_border" align="right"><?php echo $qty_perbrg ;?></td>
						
				</tr>
			
			<tr class="border">
				<td class="use_border" colspan="5"><b>Jumlah Keseluruhan : </b></td>
				<td class="use_border" align="right"><b><?php if(isset($row->total)){ echo array_sum($sum_qty);} ?></b></td>
			</tr>
	</table>
</div>
</form>
	

</div>