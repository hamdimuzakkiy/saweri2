...
<script type="text/javascript">
	...
	$(document).ready(function() {
		//$("#getbarang").fancybox();
            ...
            $.ajax({
                url: site+"/laporan_penjualan/get_barang/",
    ...
    function searchfunction(str) {
            $.ajax({
                url: site+"/laporan_penjualan/search_barang/"+str,
                success: function(response){			
                $('#jero_class').html(response);
                },
				dataType:"html"  		
	...
	function laporan(title, w, h){
		...
		if ((periode_awal=='')||(periode_akhir=='')){
			...
			// change form info:
			myForm.target = title;
			myForm.action = "<?=site_url();?>/laporan_penjualan/view_lap_penjualan_barang/v_html";
...
<section class="grid_12">
	<div class="block-border">
		...
		?>
			<h1>Form Laporan Penjualan [Barang]</h1>
